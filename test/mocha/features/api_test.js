  ...
  describe('externally fired events', () => {
      ...
      it('activates an annotation when triggered', done => {
        ...
        plugin.onReady(() => {
          player.play().then(() => {
            ...
            const openId = plugin.annotationState.annotations[0].id;
    ...
    describe('newAnnotation', () => {
      ...
      it('adds an annotation when triggered', done => {
        ...
        plugin.onReady(() => {
          player.play().then(() => {
            const startingLength = plugin.annotationState.annotations.length;
            ...
            expect(plugin.annotationState.annotations[0].commentList.comments[0].body).to.equal(
              'yeoooop'
            );
      ...
      it('adds a new comment to an Annotation', done => {
        ...
        plugin.onReady(() => {
          player.play().then(() => {
            ...
            expect(commentList.comments[commentList.comments.length - 1].body).to.equal(
              'My new comment'
            );
  ...
  describe('internally fired events', () => {
    describe('onStateChanged', () => {
      ...
      it('is triggered when plugin state is changed', done => {
        ...
        plugin.onReady(() => {
          const firstAnnotation = plugin.annotationState.annotations[0];
      ...
      it('is triggered during the add annotation process when a selectable shape is changed', done => {
        ...
        plugin.onReady(() => {
          player.play().then(() => {
            ...
            const shapeOffsetTop = $(player.el()).offset().top + 20;
            const shapeOffsetLeft = $(player.el()).offset().left + 20;
            const mockedDragEvent = {
              pageY: shapeOffsetTop,
              pageX: shapeOffsetLeft
            };
            const mockedShape = { x1: null, x2: null, y1: null, y2: null };
            const stubbedSetDims = () => {};
            const shape = plugin.controls.selectableShape;
      ...
      it('is triggered during the add annotation process when a draggable marker is changed', done => {
        ...
        plugin.onReady(() => {
          player.play().then(() => {
            ...
            // mock click and drag behavior
            const markerOffsetLeft = $(player.el()).offset().left + 250;
            const mockedDragEvent = { pageX: markerOffsetLeft };
            const mockedRange = { start: null, end: null };
            const { marker } = plugin.controls;
      ...
      it('is triggered during the add annotation process when the start of the annotation is moved', done => {
        ...
        plugin.onReady(() => {
          player.play().then(() => {
            ...
            // mock range
            const mockedRange = { start: 10, end: 20 };
            const { marker } = plugin.controls;
